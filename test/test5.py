...
quiz_name_label = tk.Label(menu_frame, text="Quiz Name", font=("Cambria", 36), fg=text_color, bg=bg_color)
quiz_name_label.pack(pady=20)

created_by_label = tk.Label(menu_frame, text="Created by $anish", font=("Cambria", 16), fg=text_color, bg=bg_color)
created_by_label.pack()

start_button = tk.Button(menu_frame, text="Start Quiz", command=start_quiz, font=("Arial", 24), fg=text_color, bg=bg_color)
start_button.pack()

# Create and configure widgets for the quiz.
question_label = tk.Label(root, text="", font=("Arial", 24), fg=text_color, bg=bg_color)
question_label.pack(pady=20)

option_var = tk.StringVar()
option_labels = []
for i in range(4):
    option_label = tk.Label(root, text="", font=("Arial", 20), fg=text_color, bg=bg_color)
    option_label.pack()
    option_labels.append(option_label)
    option_radio = tk.Radiobutton(root, variable=option_var, value=questions[0]["options"][i])
    option_radio.pack()

submit_button = tk.Button(root, text="Submit", command=check_answer, font=("Arial", 20), fg=text_color, bg=bg_color)
