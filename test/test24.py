...
def start_quiz():
    ...
    if user_name:
        ...
        messagebox.showwarning("Enter Your Name", "Please enter your name before starting the game.")
...
def check_answer():
    ...
    if current_question < len(questions):
        ...
        display_result()  # Show the result interface.
...
# Function to display the result interface.
def display_result():
    question_frame.pack_forget()  # Hide the question interface.

    result_frame = tk.Frame(root, bg=bg_color)
    result_frame.pack(pady=100, anchor="center")  # Centered the result interface within the window.

    if score == len(questions):
        result_text = f"{user_name}, Your score is {score}/{len(questions)}. Well done!"
    else:
        result_text = f"You should go back to school, {user_name}. Your score is {score}/{len(questions)}."

    result_label = tk.Label(result_frame, text=result_text,
                            font=("Cambria", text_size), fg=text_color, bg=bg_color)
    result_label.pack()

    quit_button = tk.Button(result_frame, text="Quit", command=root.quit, font=("Arial", text_size, "underline"), fg=text_color, bg=bg_color)
    quit_button.pack(pady=20)

# Function to change text size
def change_text_size():
    global text_size
    new_text_size = simpledialog.askinteger("Text Size", f"Enter the new text size:")
    if new_text_size:
        if min_text_size <= new_text_size <= max_text_size:
            text_size = new_text_size
            set_text_sizes()
        else:
            messagebox.showwarning("Text Size Limit", f"Text size must be between {min_text_size} and {max_text_size}.")
...
start_game_button.pack(side="left", padx=10)  # Added padding to the left.
...
settings_button.pack(side="left", padx=10)  # Added padding to the left.
...
question_number_label = tk.Label(root, text=f"Question 1 of {len(questions)}", font=("Cambria", text_size - 6), fg=text_color, bg=bg_color, anchor="e")
